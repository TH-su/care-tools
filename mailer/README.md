...
- **`The OAuth client was not found.` と出る** — Googleがクライアントを認証できなかったときのメッセージで、
  実際には**シークレットの不一致**であることがほとんどです（IDが存在しない場合と同じ文言が返るため紛らわしい）。
  接続画面の **「設定を確認」** を押すと、ID・シークレット・リダイレクトURIのどれが原因か切り分けられます。
  「シークレットを追加」で新しく発行し、コピーアイコンで取得して貼り直してください
