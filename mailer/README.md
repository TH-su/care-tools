...
- **`The OAuth client was not found.` と出る** — Googleがクライアントを認証できなかったときのメッセージです。
  失敗した画面に、**アプリの版・クライアントID・PKCEの有無・リダイレクトURI**が残ります（この画面は自動で閉じません）。
  接続画面の **「設定を確認」** を押すと、ID・シークレット・リダイレクトURIのどれが原因か切り分けられます。
  多いのはシークレットの不一致で、その場合は「シークレットを追加」で新しく発行し、
  コピーアイコンで取得して貼り直してください（**「表示」で実際に入っている値を確認**できます。
  ブラウザのパスワード管理が古い値を入れていることがあります）
