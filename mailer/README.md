...
### まず `npm run diag`（Google連携がうまくいかないとき）

原因を一か所ずつ手で確かめる必要はありません。次の1コマンドが、設定の控え・Googleへの
問い合わせ・連携済みカレンダーの疎通を**実際にGoogleへ送って**順に確かめ、
どこで止まっているかと、次にやることを出します。
